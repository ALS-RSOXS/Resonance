...
class BCSServer:
    ...
    * **log** (*bool*) - True if the server logged this request.
    ...
    async def connect(self, addr="127.0.0.1", port=5577):
        ...
        if not _zmq_context:
            if (
                "zmq.asyncio" in sys.modules
            ):  # Using asyncio? (was the module imported?)
    ...
    async def close_display(self, name="Display Name") -> dict:
        """
        Sets the digital output (DO) channel  ``chan`` to ``value``

        :param name: Name of the channel to set.
        :type name: str
        ...
        """
        return await self.bcs_request("CloseDisplay", dict(locals()))
    ...
    async def command_motor(self, commands=[], motors=[], goals=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **timed_out** (*list[str]*) - List of motors that timed-out, if any.

            * **not_found** (*list[str]*) - List of motors that were not found, if any.
    ...
    async def get_acquired(self, chans=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **chans** (*list[str]*) - AI channel names that have data in the **data** array.
    ...
    async def get_acquired_array(self, chans=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **not_found** (*list[str]*) - The requested channels in ``chans`` that were not found on the server, if any.

            * **chans** (*list[dict]*) - Array of clusters/dictionaries. Each array element contains the channel (**chan**), **period**, **time**, and **data**. Data is an array of sampled values.
    ...
    async def get_bcsconfiguration(self) -> dict:
        """
        Read the device configurations (the setup files) and return them in JSON. If a subsystem is mission from the output, it may mean that the setup has not been saved, or that it is not yet supported.



        :return: Dictionary of results, with the following key(s).

            * **configuration** (*str*) - JSON containing keys ``motor``, ``ai``, each of which contains their respective configuration (the data from the xx setup.txt files) in JSON.


        """
        return await self.bcs_request("GetBCSConfiguration", dict(locals()))
    ...
    async def get_di(self, chans=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **chans** (*list[str]*) - DI channel names that have data in the **data** array.

            * **not_found** (*list[str]*) - The requested channels in ``chans`` that were not found on the host system, if any.

            * **enabled** (*list[bool]*) - Boolean list, in the same order as **chans**, indicating if the channel is used (enabled).

            * **data** (*list[bool]*) - Boolean value of the DI's, in the same order as the channel names in **chans**.


        """
        return await self.bcs_request("GetDI", dict(locals()))

    async def get_display_status(self, name="Display Name") -> dict:
        """
        Returns the status of the display named ``name``. There may be multiple displays with the same name.
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **exists** (*list[bool]*) - Whether the display is defined to BCS

            * **status** (*list[str]*) - One of {invalid, standard, closed, hidden, minimized, maximized}


        """
        return await self.bcs_request("GetDisplayStatus", dict(locals()))
    ...
    async def get_flying_positions(self, name="") -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **pulse_positions** (*list[float]*) - Trigger (pulse) positions.


        """
        return await self.bcs_request("GetFlyingPositions", dict(locals()))

    async def get_folder_listing(
        self, path=".", pattern="*.txt", recurse=False
    ) -> dict:
        """
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **files** (*list[str]*) - Array of paths, relative to C:\\\\Beamline Controls\\\\BCS Setup Data.

            * **folders** (*list[str]*) - Array of paths, relative to C:\\\\Beamline Controls\\\\BCS Setup Data.
    ...
    async def get_freerun(self, chans=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **chans** (*list[str]*) - The retrieved channel names, in the same order as **data**.

            * **not_found** (*list[str]*) - The channels requested in ``chans`` that were not found on the server.

            * **data** (*list[float]*) - The retrieved data, corresponding to the channels in **chans**.
    ...
    async def get_freerun_array(self, chans=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **chans** (*list[dict]*) - Array of clusters/dictionaries. Each array element contains the channel (**chan**), **data**, and **x_values**.
    ...
    async def get_instrument_acquired1d(self, name="") -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **x** (*list[float]*) - 1D array of abscissae

            * **y** (*list[float]*) - 1D array of data
    ...
    async def get_instrument_acquired2d(self, name="") -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **data** (*list[int]*) - 2D array of u32.


        """
        return await self.bcs_request("GetInstrumentAcquired2D", dict(locals()))

    async def get_instrument_acquired2d_string(self, name="") -> dict:
        """
        Retrieve data (2D array) from the most recent acquisition.
        ...
        """
        return await self.bcs_request("GetInstrumentAcquired2DString", dict(locals()))

    async def get_instrument_acquired2d_base85(self, name="") -> dict:
        """
        Retrieve data (2D array) from the most recent acquisition.
        ...
        """
        return await self.bcs_request("GetInstrumentAcquired2DBase85", dict(locals()))
    ...
    async def get_instrument_acquisition_info(self, name="") -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **live_time** (*list[float]*) - Live time for the acquistion.

            * **dead_time** (*list[float]*) - Dead time for the acquisition.
    ...
    async def get_instrument_count_rates(self, name="") -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **input_cr** (*list[float]*) - Input count rate

            * **output_cr** (*list[float]*) - Output count rate
    ...
    async def get_motor(self, motors=[]) -> dict:
        """
            ...
            :return: Dictionary of results, with the following key(s).
                ...
                * **not_found** (*list[str]*) - The requested motors in ``motors`` that were not found on the host system, if any.

                * **data** (*list[dict]*) - Array of clusters/dictionaries. Each array element contains the motor name (**motor**), **position**, **position_raw**, **goal**, **goal_raw**, **status**, and **time**.
    ...
    async def get_motor_full(self, motors=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **not_found** (*list[str]*) - Names in **motors** that were not found, if any.

            * **data** (*list[dict]*) - dictionary of dictionaries all relateing to state. Too many parameters to list atm.
    ...
    async def get_state_variable(self, name="") -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **type** (*str*) - 'String', 'Boolean', 'Integer', or 'Double'

            * **name** (*str*) - Same as ``name``

            * **numeric_value** (*float*) - Value for Boolean, Integer, and Float variables

            * **string_value** (*str*) - Value for String variables
    ...
    async def get_subsystem_status(self) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **status** (*list[dict]*) - Array of dictionaryies of keys: **name** - Subsystem Name, and **status** - one of {Initializing, Running, Stopping, Stopped, Force Stop, Disabled, Bad Path, VI Broken Not in List}
    ...
    async def get_text_file(
        self,
        path="You must have at least one input control in the Parameter cluster. Delete this (_unused) if you don't need it.",
    ) -> dict:
        """
    ...
    async def home_motor(self, motors=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **timed_out** (*list[str]*) - True if the home operation timed out.

            * **not_found** (*list[str]*) - The motors in ``motors`` that are not found on the server, if any.
    ...
    async def list_ais(self) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **names** (*list[str]*) - Llist of AI channels that are defined on the server. The list includes disabled and hidden channels.

            * **displayed** (*list[str]*) - The list of AI channel that are displayed (not set to 'hidden' in BCS).

            * **disabled** (*list[int]*) - Iindexes in **displayed** that are disabled.
    ...
    async def list_dios(self) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **names** (*list[str]*) - Liist of all digital input channel names on the server. The list includes disabled and hidden channels.

            * **displayed** (*list[str]*) - The list of channels that are displayed (not set to 'hidden' in BCS).
    ...
    async def list_displays(self) -> dict:
        """
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **names** (*list[str]*) - The list of motors that are defined on the server. The list includes disabled and hidden motors.


        """
        return await self.bcs_request("ListDisplays", dict(locals()))
    ...
    async def list_instruments(self) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **names** (*list[str]*) - The list of all instruments that are defined on the server,  including disabled and hidden instruments.

            * **displayed** (*list[str]*) - The list of instruments that are displayed (not set to 'hidden' in BCS).
    ...
    async def list_motors(self) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **displayed** (*list[str]*) - The list of motors that are displayed (not set to 'hidden' in BCS).
    ...
    async def list_presets(self) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **presets** (*list[dict]*) - Array of dictionaries, one per preset, each with keys: **Preset Name**, **Motor Name**, **Preset Position**, **Tolerance (+/-)**.
    ...
    async def list_state_variables(self) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **names** (*list[str]*) - Current list of state variable on the server.

            * **types** (*list[str]*) - Variable types (Boolean, Integer, String, Double), in the same order as **names**.
    ...
    async def list_trajectories(self) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **trajectories** (*list[dict]*) - Array of dictionaries, each element describing a trajectory.


        """
        return await self.bcs_request("ListTrajectories", dict(locals()))

    async def move_all_trigger733(
        self, motors=[], goals=[], timeout_s=0, wait_for_data_saved=False
    ) -> dict:
        """
        ...
        :param wait_for_data_saved: If true, the command server will write a log entry for the execution of this command.
        :type wait_for_data_saved: bool
        ...
        """
        return await self.bcs_request("MoveAllTrigger733", dict(locals()))
    ...
    async def move_to_preset(self, names=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **not_found** (*list[str]*) - List of names in ``names`` that were not found on the server, if any.
    ...
    async def open_display(self, name="Display Name") -> dict:
        """
        Sets the digital output (DO) channel  ``chan`` to ``value``
        ...
        """
        return await self.bcs_request("OpenDisplay", dict(locals()))
    ...
    async def set_breakpoints(
        self, name="", x0=0, dx=0, n=0, breakpoints=[], counts_or_units=False
    ) -> dict:
        """
    ...
    async def start_instrument_acquire(
        self, name="", run_type="Exposure", acq_time_s=0, acq_counts=0
    ) -> dict:
        """
    ...
    async def stop_motor(self, motors=[]) -> dict:
        ...
        :return: Dictionary of results, with the following key(s).
            ...
            * **timed_out** (*list[str]*) - Motor subsystem error indicating that the command queue is unavailable. Seek help from the beamline scientist or beamline controls.

            * **not_found** (*list[str]*) - Those motors requested in ``motors`` that were not found on the server, if any.
    ...
    async def sc_alsu_mirror_vibration(
        self,
        time_sec=0,
        start_delay_sec=0,
        x_motor="",
    ...
    async def sc_auto_coll_single_axis_scan(
        self,
        direction="Forward",
        index=0,
        ...
        stop=0,
        increment=1,
        delay_after_move_s=0,
        count_time_s=0.5,
        bidirect=True,
        ai_for_min_max="Select Motor",
        at_end_of_scan="Return",
        min_max="Min",
        description="",
        file_pattern="",
    ) -> dict:
        ...
        :type index: int
        :param x_motor: Motor to move for scan
        :type x_motor: str
        :param start: start position of motor scan
        :type start: float
        :param stop: End position of motor scan
        :type stop: float
        :param increment: Step size moving motor from start to stop
        :type increment: float
        :param delay_after_move_s: Time to delay after move before starting acquire
        :type delay_after_move_s: float
        :param count_time_s: Count time in seconds
        :type count_time_s: float
        :param bidirect: If checked then even scan numbers motor moves from max to min
        :type bidirect: bool
        :param ai_for_min_max: AI to use for finding min or max
        :type ai_for_min_max: str
        :param at_end_of_scan: Determines action to take at end of scan
        :type at_end_of_scan: str
        :param min_max: At end of san selets whether to move to min or max of curve
        :type min_max: str
        :param description:
        :type description: str
        :param file_pattern:
    ...
    async def sc_barf_scan(
        self,
        instrument="",
        exposure_time_s=0.001,
        number_to_average=1,
        continuous_acquire=True,
        background_data="Q:\\Commissioning\\OffLineRuby\\background\\background2020-03-12_11-15-40.txt",
        fit_type="Lorentzian",
        ambient_peak_2=694.23,
        ambient_peak_1=692.8,
        file_index=1,
        description="",
        file_pattern="",
    ) -> dict:
        """
        The Barf scan acquires from a spectrometer and then fits the ruby peaks to estimate the pressure. The data can be viewed with the XY Graph which allows adjustment of the peak positions with cursors.
        ...
        :param file_index: File Index for user file that is used in place of scan number.
        :type file_index: int
        :param description:
        :type description: str
        :param file_pattern:
    ...
    async def sc_from_file_scan(
        self,
        file_path="",
        delay_after_move_s=0,
        count_time_s=0.5,
        at_end="Return",
        move_motors_sequentially=False,
        dont_repeat_motor_moves=False,
        move_at_end_of_scan="",
        description="",
        file_pattern="",
    ) -> dict:
    ...
    async def sc_image_pyrometry_scan(
        self,
        diffraction_instrument="",
        exposure_sec=599.5,
        file_index=0,
        instrument="",
        exposure_time_msec="",
        ...
        lambda_max=0,
        lambda_min=0,
        nd_filter_motor="Select Motor",
        instrument_2="",
        exposure_time_msec_2="",
        calibration_temperature_2="",
        xmin_2=0,
        xmax_2=0,
        lambda_max_2=0,
        lambda_min_2=0,
        nd_filter_motor_2="Select Motor",
        instrument_3="",
        roi_size=0,
        temp_background_k=0,
        instrument_4="",
        roi_size_2=0,
        temp_background_k_2=0,
        ...
        :param diffraction_instrument: Instrument name for upstream 2D
        :type diffraction_instrument: str
        :param exposure_sec:
        :type exposure_sec: float
        :param file_index: File Index for user file used in place of scan number.
        :type file_index: int
        :param instrument:
        :type instrument: str
        ...
        :param instrument_4: Instrument name for upstream 2D
        :type instrument_4: str
        :param roi_size_2: Size of ROI around hot spot
        :type roi_size_2: float
        :param temp_background_k_2: Background Temperature for contour plot
        :type temp_background_k_2: float
        :param acquire_images: Selects whether to also acquire the 2d images
        :type acquire_images: bool
        :param file_index_2: Index of file to save.
        :type file_index_2: int
        :param fit_save: Click after adjusting fit range to refit and save.
        :type fit_save: bool
        :param description:
        :type description: str
        :param file_pattern:
        ...
        """
        return await self.bcs_request("sc_Image Pyrometry Scan", dict(locals()))

    async def sc_ltp_single_axis_scan(
        self,
        direction="Forward",
        index=0,
        instrument="",
        frames_to_average=0,
        delay_between_frames_s=0,
        x_motor="Select Motor",
        start=-3,
        stop=0,
        increment=1,
    ...
    async def sc_mca_single_motor_scan(
        self,
        x_motor="Select Motor",
        start=-3,
        stop=0,
        ...
        """
        return await self.bcs_request("sc_MCA Single Motor Scan", dict(locals()))

    async def sc_move_motor(
        self,
        motor_name="",
        motor_value=0,
        delay_sec=0,
        motor_action="Normal Move",
        delay_type="Delay from Start",
        description="",
        file_pattern="",
    ) -> dict:
        """
        ...
        :param motor_name: Choose the motor to move.
        :type motor_name: str
        :param motor_value: Select the value of the motor move.
        :type motor_value: float
        :param delay_sec: Configure a Delay for the move. The delay can be configured to happen before or after the move.
        :type delay_sec: float
        :param motor_action: Select the type of move to make
        :type motor_action: str
        :param delay_type: Configure when the delay will occure.
        :type delay_type: str
        :param description:
        :type description: str
        :param file_pattern:
    ...
    async def sc_move_trajectory(
        self,
        trajectory_name="",
        delay_after_move_sec=0,
        description="",
        file_pattern="",
    ) -> dict:
        """
    ...
    async def sc_multiple_single_motor_scan(
        self,
        x_motor="Select Motor",
        delay_after_move_s=0,
        steps=[],
        ...
        :param x_motor:
        :type x_motor: str
        :param delay_after_move_s:
        :type delay_after_move_s: float
        :param steps:
        :type steps: list[float]
        :param description:
        :type description: str
        :param file_pattern:
        :type file_pattern: str
        ...
        """
        return await self.bcs_request("sc_Multiple Single Motor Scan", dict(locals()))

    async def sc_powder_diffraction(
        self,
        exposure_time=0,
        number_of_samples=0,
        file_index=0,
    ...
    async def sc_save_trajectory(
        self,
        base_trajectory_name="",
        ...
        :param base_trajectory_name:
        :type base_trajectory_name: str
        :param delete_older_than_days:
        :type delete_older_than_days: float
        :param motors_to_save:
        :type motors_to_save: str
        :param description:
        :type description: str
        :param file_pattern:
        :type file_pattern: str
        ...
        """
        return await self.bcs_request("sc_Save Trajectory", dict(locals()))

    async def sc_set_dio(
        self,
        dio_name="",
        delay_after_move_sec=0,
        state="ON",
        ...
        """
        This Scan simply turns the selected DIO on or off and then waits for the specified time. Used it Automation tool to do things like open/close shutters.

        :param dio_name: Name of the DIO to turn on or off.
        :type dio_name: str
        :param delay_after_move_sec: Delay after turning DIO on or off before running next step if using automationi.
        :type delay_after_move_sec: float
        :param state: Determines if the selected DIO is turned on or off when the scan is run.
        :type state: str
        :param description:
        :type description: str
        :param file_pattern:
    ...
    async def sc_set_instrument(
        ...
        live=False,
        dark_image_file="",
        sync_with_ai_acq=False,
        exposure_time_s=0,
        subtract_dark_from_saved_image=False,
        number_to_sum=1,
        current_of_sum=1,
        counts=1,
        description="",
        ...
        :param instrument: Instrument to acquire
        :type instrument: str
        :param instrument_setup: Open Instrument Setup Panel
        :type instrument_setup: bool
        :param save_as:
        :type save_as: str
        :param take_picture: A single image is acquired and displayed when clicked.
        :type take_picture: bool
        :param live: When checked images are acquired from the selected instrument.
        :type live: bool
        :param dark_image_file: To acquire a dark image, run a scan when the camera is dark. Then choose a particular image of that scan as your dark image.
        :type dark_image_file: str
        :param sync_with_ai_acq: When checked images are acquired from the selected instrument.
        :type sync_with_ai_acq: bool
        :param exposure_time_s:
        :type exposure_time_s: float
        :param subtract_dark_from_saved_image:
        :type subtract_dark_from_saved_image: bool
        :param number_to_sum:
        :type number_to_sum: int
        :param current_of_sum:
        :type current_of_sum: int
        :param counts:
        :type counts: int
        :param description:
        :type description: str
        ...
        """
        return await self.bcs_request("sc_Set Instrument", dict(locals()))

    async def sc_single_crystal_scan(
        self,
        instrument="",
        save_images_as="",
        x_motor="",
    ...
    async def sc_single_motor_flying_scan(
        self,
        x_motor="",
        start=0,
        stop=10,
    ...
    async def sc_single_motor_relative_scan(
        self,
        x_motor="Select Motor",
        distance=-3,
        current_position=0,
        ...
        min_max="Min",
        percent=0,
        set_feedback_motor=False,
        feedback_motor="Select Motor",
        smooth_ai_pixels=2,
        description="",
        file_pattern="",
    ) -> dict:
        """
        Setup the single motor relative scan.
        This scan moves a single motor to multiple positions at regular intervals and takes data at each spot. The motor that is moved is recorded and is set as the default x axis when this data is graphed on an xy graph.

        :param x_motor: Motor to scan.
        :type x_motor: str
        :param distance: Distance to move scanning motor in positive and negative direction relative to current position.
        :type distance: float
        :param current_position: Current position of scanning motor.
        :type current_position: float
        :param increment: Step size for scanning motor.
        :type increment: float
        :param delay_after_move_s: Time to delay after motor move before reading AIs.
        :type delay_after_move_s: float
        :param count_time_s: Time to measure AI at each point.
        :type count_time_s: float
        :param number_of_scans: Number of scans to do.
        :type number_of_scans: int
        :param bidirect: Scan in both directions?
        :type bidirect: bool
        :param ai_for_min_max: AI used for finding min or max position.
        :type ai_for_min_max: str
        :param at_end_of_scan: Action to take at the end of the scan.
        :type at_end_of_scan: str
        :param min_max: Find Min or Max?
        :type min_max: str
        :param percent: Percent of peak to move to and set feedback motor to.
        :type percent: float
        :param set_feedback_motor: Use Feedback motor?
        :type set_feedback_motor: bool
        :param feedback_motor: Optional feedback motor to set to value at end of scan.
        :type feedback_motor: str
        :param smooth_ai_pixels: Smoothing to apply to AI before finding min/max.
        :type smooth_ai_pixels: float
        :param description:
        :type description: str
        :param file_pattern:
        :type file_pattern: str
        ...
        """
        return await self.bcs_request("sc_Single Motor Relative Scan", dict(locals()))

    async def sc_single_motor_scan(
        self,
        x_motor="Select Motor",
        start=-3,
        stop=0,
    ...
    async def sc_spin_coater_scan(
        self,
        num_stop_steps=0,
        ai_exposure_time_s=0.5,
        steps=[[]],
        instrument="",
        exposure_sec=0,
        description="",
        ...
        :param num_stop_steps:
        :type num_stop_steps: int
        :param ai_exposure_time_s: Specify how long to sample the input data for. All the data sampled during this time will be averaged together, depending on the data source.
        :type ai_exposure_time_s: float
        :param steps:
        :type steps: list[str]
        :param instrument:
        :type instrument: str
        ...
        """
        return await self.bcs_request("sc_Spin Coater Scan", dict(locals()))

    async def sc_tcp_command(self, description="", file_pattern="") -> dict:
        """
        ...
        """
        return await self.bcs_request("sc_TCP Command", dict(locals()))

    async def sc_temperature_scan(
        self,
        instrument="",
        exposure_time_msec="",
        calibration_temperature="",
        ...
        """
        The Temperature Scan acquires data from a spectrometer and then with background and calibration data calculates the temperature. It also optionally acquires an image from a 2D instrument and then displays a contour plot of the temperature profile along with recomputing the temperature based on the 2D image.
        The fit range of the temperature data can be adjusted with the Cursors on the Pyrometry Graphs. Adjusting the fit range in the Scan setup will also adjust the position of the cursors.
        ...
        :param acquire_images: Selects whether to also acquire the 2d images
        :type acquire_images: bool
        :param file_index: Index of file to save.
        :type file_index: int
        :param fit_save: Click after adjusting fit range to refit and save.
        :type fit_save: bool
        :param description:
    ...
    async def sc_time_scan(
        self,
        total_time_s=0,
        number_of_samples=0,
        time_between_points_s=0,
        count_time_s=0,
        frequency_hz=0,
        stop_condition="Samples Taken",
        description="",
        file_pattern="",
    ) -> dict:
        """
        ...
        :param total_time_s: Total time for scan.
        :type total_time_s: float
        :param number_of_samples: Number of samples to acquire.
        :type number_of_samples: int
        :param time_between_points_s: Time between acquisitions. If more than exposure time, delay between exposures.
        :type time_between_points_s: float
        :param count_time_s: Exposure time.
        :type count_time_s: float
        :param frequency_hz: Sample acquisition frequency.
        :type frequency_hz: float
        :param stop_condition: End of scan criteria.
    ...
    async def sc_trajectory_scan(
        ...
        :param stop_trajectory: A trajectory to be called at the end of the scan. Trajectories can be setup in the motor system. The BCS team can help. If the trajectory is left blank, nothing will be done.
        :type stop_trajectory: str
        :param description:
        :type description: str
        :param file_pattern:
        :type file_pattern: str
    ...
    async def sc_two_motor_scan(
        ...
        :param x_motor: X Motor to scan
        :type x_motor: str
        :param x_start: Start of scan range for X motor
        :type x_start: float
        :param x_stop: End of scan range for X motor
        :type x_stop: float
        :param x_increment: Step size for X motor
        :type x_increment: float
        :param delay_after_move_s: Time to delay after move is complete before doing acquisition
        :type delay_after_move_s: float
        :param count_time_s: Count time for acquisition at each point of scan
        :type count_time_s: float
        :param y_motor: Y motor to scan
        :type y_motor: str
        :param y_start: Start of scan range for Y motor
        :type y_start: float
        :param y_stop: End of Scan range for Y motor
        :type y_stop: float
        :param y_increment: Step size for Y motor
        :type y_increment: float
        :param at_end_of_scan: Select whether scan motors stay at end position or return to start position at the end of the scan
        :type at_end_of_scan: str
        :param description:
        :type description: str
        :param file_pattern:
    ...
    async def sc_video_scan(
        ...
        :param video_names:
        :type video_names: str
        :param video_names_2:
        :type video_names_2: str
        :param video_names_3:
        :type video_names_3: str
        :param total_time_s: Total time for scan.
        :type total_time_s: float
        :param number_of_samples: Number of samples to acquire.
        :type number_of_samples: int
        ...
        """
        return await self.bcs_request("sc_Video Scan", dict(locals()))
